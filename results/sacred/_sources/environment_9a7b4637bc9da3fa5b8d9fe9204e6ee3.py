...
class MultiAgentEnv(gym.Env):
    ...
    def step(self, action_n):
        ...
        if self.has_saved == False:
            ...
            """ print('obs_n[0]')
            print(obs_n[0]) """
            self.game_history.append(obs_n[0]) # each agent's observation represents entire world state (eventually will do partial observation)
    ...
    def reset(self):
        ...
        # save
        if self.has_saved == False and self.game_history != []:
            self.has_saved = True
    ...
    def _set_action(self, action, agent):
        # action array is softmax with each value representing one positiliby in action space
        agent.direction_to_change = np.argmax(action[0:9])
        if action[9] > action[10]: # action[9] means throw
            agent.throw = True
        else:
            agent.throw = False
        agent.target_player_num = np.argmax(action[11:18]) - 11
    ...
    # render environment (I have not changed any of this code)
